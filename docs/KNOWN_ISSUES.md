...
설명:
- 마이 화면의 공지/문의 영역은 운영 준비 안내 문구만 제공하며, 실제 연결 기능은 아직 없다.
...
대응:
- `docs/08_운영배포_환경분리_가이드.md` 와 `python backend_api/scripts/check_release_readiness.py` 순서대로 환경 분리와 smoke test 를 먼저 진행한다.
