...
This repository is already using a stable mono-repo structure. For this repo, the **current real structure is the final structure**.
...
- `admin_web`: local MVP admin/backoffice web UI
- Flutter app: repository root (`lib/`, `android/`, `ios/`, `web/`, `test/`, `pubspec.yaml`)
- `docs`: product, logic, DB/API, operations documents

Do not move or rename the documentation files unless explicitly instructed.

Do not force a `frontend_app/` move unless a human explicitly asks for a migration.
...
### Mobile App
- Flutter
- feature-based folder structure inside the current root Flutter app
...
Prefer feature-based structure inside the current root Flutter app:
